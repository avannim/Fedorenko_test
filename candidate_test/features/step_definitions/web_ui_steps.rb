...
When(/^удаляю пользователя с логином i\.ivanov из списка пользователей$/) do
  pending
end

When(/^пользователю с логином f\.akelogin передаю имя testing фамилию task$/) do
  pending
end

When(/^нахожу пользователя с логином f\.akelogin именем testing фамилией task$/) do
  pending
end